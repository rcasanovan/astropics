...
extension AstronomyPictures.State {
  @MainActor static let loading = Self(networkState: .loading)

  @MainActor static let success = Self(networkState: .completed(.success(.mock)))

  @MainActor static let failure = Self(networkState: .completed(.failure(.cannotLoadPictures(error: "error"))))

  @MainActor static let refreshing = Self(networkState: .completed(.success(.mock)), isRefreshing: true)
}

extension Array where Element == AstronomyPictureModel {
